...
$('.waifu-tool .drivers-license-o').click(function(){
    if(model_p===22){
        loadlive2d('live2d',`${apiurl}22-${m22_id}.json`);
        model_p = 33;
        showMessage('33援交有点累了，现在该我上场了',4000)
    }else{
        loadlive2d('live2d',`${apiurl}33-${m33_id}.json`);
        model_p = 22;
...
$('.waifu-tool .street-view').click(function (){
    if(model_p===22){
        ...
        m22_id += 1;
        loadlive2d('live2d',`${apiurl}22-${m33_id}.json`)
    }
    showMessage('我的新衣服好看嘛',4000);
});
loadlive2d('live2d',`${apiurl}33-0.json`);
